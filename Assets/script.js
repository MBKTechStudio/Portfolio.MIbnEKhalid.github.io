...
async function AskForCookieConsent() {
	try {
		const response = await fetch('https://portfolio.mbktechstudio.com/Assets/cookie.html');
		const html = await response.text();
...
function setCookie(name, value, days) {
	...
	var Pdomain = "; domain=.mbktechstudio.com";
	var Sdomain1 = "; domain=.mbktech.xyz";
	var Sdomain2 = "; domain=.ibnekhalid.me";
	document.cookie = name + "=" + (value || "") + expires + "; path=/" + Pdomain;
	document.cookie = name + "=" + (value || "") + expires + "; path=/" + Sdomain1;
	document.cookie = name + "=" + (value || "") + expires + "; path=/" + Sdomain2;
}
